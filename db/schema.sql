...
--                  [DROPS]                   --
------------------------------------------------
DROP TABLE IF EXISTS manga_alt_titles CASCADE;
DROP TABLE IF EXISTS chapter_metrics CASCADE;
...
CREATE TABLE IF NOT EXISTS manga_comments (
    ...
    FOREIGN KEY (parent_id) REFERENCES manga_comments(id) ON DELETE CASCADE,
...
BEGIN
    ...
    IF parent_path IS NULL THEN
        RAISE EXCEPTION 'Parent comment % not found or has no path', NEW.parent_id;
    END IF;
...
CREATE OR REPLACE TRIGGER trg_set_comment_path
AFTER INSERT ON manga_comments
FOR EACH ROW
